...
- History display can show previously recorded MT5 orders for a chosen period, marking records that are still in Trade History and allowing deleted ones to be recorded again.
- MT5 broker-server timestamps are normalized before display. Realtime and History use separate MT5 time corrections because brokers can encode current closed-position time and history time differently.
